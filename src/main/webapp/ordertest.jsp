          ...
          <form method="get" action="order">
            <ul class="nav navbar-nav mr-auto">
              <li class="dropdown nav-item" style="margin-left: 10px">
                <select name="status" class="h-100 border-0"
                  ...
                  <c:forEach var="order" items="${listOrder}">
                    <option class="dropdown-item" ${String.valueOf(order.getStatus()).equals(param.status) ? "selected" : ""}
                            value="${order.getStatus()}">${order.getStatus()}</option>
                  </c:forEach>
                </select>
              </li>
              ...
              <li class="nav-item" style="margin-left: 10px">
                <input type="text" name="search" placeholder="Tìm kiếm " class="h-50 border-0"
                       value="${param.search}">
